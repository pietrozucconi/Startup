...
$componentFile = Join-Path $repoRoot 'components\StartupBrain3D.tsx'
$apiFile = Join-Path $repoRoot 'app\api\brain\topology\route.ts'

if (-not (Test-Path (Join-Path $repoRoot 'package.json'))) {
    throw 'Run this script from the Startup repository root.'
}

if (-not (Test-Path $componentFile)) {
    throw 'components\StartupBrain3D.tsx was not found. Install the first 3D checkpoint before this update.'
}

Write-Host '=== Startup Brain 3D - visual architecture V2 ===' -ForegroundColor Cyan
Write-Host 'No GitHub commit or push will be performed.' -ForegroundColor DarkGray

$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
Copy-Item $componentFile "$componentFile.$stamp.bak"
if (Test-Path $apiFile) {
    Copy-Item $apiFile "$apiFile.$stamp.bak"
}

Write-Host '[1/3] Updating topology contract...'
@'
...
Write-Host '[2/3] Rebuilding the interactive 3D scene...'
@'
'use client';
  ...
  return (
    <div className="relative overflow-hidden rounded-lg-t border border-os-border bg-[#03050a]">
      ...
      <div className="absolute bottom-4 left-4 z-10 flex flex-wrap gap-2 rounded-md-t border border-os-border bg-black/75 px-3 py-2 text-[10px] backdrop-blur">
        ...
        <span className="text-os-dim">drag = rotate · wheel = zoom · right-drag = pan · no auto-reset</span>
      </div>
...
Write-Host '[3/3] Running TypeScript validation...'
& npm.cmd run typecheck
if ($LASTEXITCODE -ne 0) {
    throw 'Typecheck failed. Send the terminal output and do not continue until it is fixed.'
}

Write-Host ''
Write-Host 'Startup Brain visual architecture V2 installed successfully.' -ForegroundColor Green
Write-Host 'Refresh: http://localhost:4100/brain' -ForegroundColor Cyan
Write-Host 'This update does not add or connect any external tools.' -ForegroundColor DarkGray
